...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔑 الحل 1: تحديد مسار آمن لا يحتوي على أحرف عربية
    ...
    file_path = f"/tmp/{file_id}{extension}" # المسار الآن آمن
    ...
    # 🔑 الحل 2: تأمين تسجيل الأخطاء
    except Exception as e:
        # تحويل الخطأ إلى تشفير آمن وإهمال الأحرف المشكلة في السجلات
        error_message = str(e).encode('utf-8', errors='ignore').decode('utf-8')
        logging.error(f"Error processing file: {error_message}")
        
        # حذف الملف المحلي لضمان التنظيف
        if os.path.exists(file_path):
            os.remove(file_path)
            
        # إرسال رسالة خطأ عامة للمستخدم (رسالة قصيرة وآمنة)
