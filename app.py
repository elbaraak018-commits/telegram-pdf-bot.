...
# البرومبت الخاص بمعالجة الملفات (يحتوي على طلب الأسئلة والأمثلة)
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔑 التعديل المطلوب: رسالة جاري المعالجة
    ...
    try:
        ...
        # 5. توليد المحتوى (باستخدام برومبت الملفات)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالجة النصوص العامة والأسئلة (الآن يجيب على النص فقط دون توليد أسئلة وأمثلة)"""
    ...
    # 🔑 التعديل المطلوب: رسالة جاري التحليل
    msg = await update.message.reply_text("🤔") 
    
    try:
        # 🔑 التعديل الرئيسي: لا نستخدم برومبت الملفات هنا. نعتمد على برومبت نظام بسيط.
        response = client.models.generate_content(
            model='gemini-2.5-flash',
            contents=[f"أنت بوت معلم واسمك البراء. أجب على طلب المستخدم أو حلل النص المقدم بأسلوب تعليمي ومزود بالإيموجي. \n\nالنص: {update.message.text}"]
        )
        response_parts = split_text(response.text)
        await msg.delete() 
        for i, part in enumerate(response_parts):
