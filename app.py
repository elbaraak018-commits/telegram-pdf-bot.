...
def create_pdf_from_text(content, filename="EduVise_Explanation.pdf"):
    """دالة محسنة لتحويل النص العربي إلى ملف PDF"""
    try:
        c = canvas.Canvas(filename, pagesize=A4)
        width, height = A4
        
        # تسجيل خط يدعم العربية (يجب أن يكون ملف arial.ttf موجوداً في نفس المجلد)
        font_path = "arial.ttf"
        try:
        ...
        for line in lines:
            ...
            for w_line in wrapped_lines:
                ...
                # الرسم من اليمين (لأن النص عربي)
                c.drawRightString(width - 50, y, w_line)
                y -= 20
        c.save()
        return filename
    except Exception as e:
