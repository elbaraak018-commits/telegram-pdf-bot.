...
async def get_users_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تعديل العنوان ليشمل معرف المستخدم لضمان ظهور القائمة بشكل صحيح
    response += "📋 **آخر 50 اسم مسجل (معرف المستخدم):**\n"
    
    for user_id, first_name, username in users_list:
        if username:
            response += f"- {first_name} (@{username}) (ID: {user_id})\n"
        else:
            response += f"- {first_name} (ID: {user_id})\n"
...
async def receive_broadcast_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id in user_ids:
        try:
            # استخدام دالة forward_message للحفاظ على تنسيق الملفات
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        while time.time() - start_time < MAX_WAIT_TIME:
            ...
            try:
                # محاولة تحديث النسبة المئوية
                await status_msg.edit_text(f"⏳ {progress_percent}%") 
            except Exception as edit_e:
                # تجاوز خطأ عدم تعديل الرسالة إذا لم يتغير المحتوى
                pass
        ...
        # 🔑 التحديث النهائي لرسالة الحالة يتم هنا بعد نجاح التوليد
