        ...
        try:
            ...
            if "429" in error_msg or "rate_limit_exceeded" in error_msg:
                ...
                if attempt == retries - 1:
                    wait_time = "قليلاً"
                    # محاولة استخراج الوقت الدقيق من رسالة الخطأ (مثل 5s أو 452ms)
                    match = re.search(r"try again in ([\d\.]+\w+)", error_msg)
                    if match: wait_time = match.group(1)
                    
                    # الرد النهائي بالوقت المحدد
...
async def media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if mode == "pdf":
            ...
            # تم تقليل عدد الأحرف المستخرجة قليلاً لتقليل استهلاك التوكنز في الطلب الواحد
            ...
            # تخزين النص في بيانات المستخدم لاستخدامه عند ضغط الزر
            context.user_data[f"audio_text_{msg.from_user.id}"] = transcription
            
            keyboard = [
                [InlineKeyboardButton("📝 المحتوى النصي", callback_data="audio_show_text")],
                [InlineKeyboardButton("🧠 شرح المحتوى", callback_data="audio_explain_text")]
            ]
