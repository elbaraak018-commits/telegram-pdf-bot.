...
# إعداد عميل Gemini (المكتبة الجديدة)
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # محاولة الحصول على اسم الملف الأصلي لضمان الامتداد الصحيح
    filename = file_obj.file_name if update.message.document else f"{file_obj.file_unique_id}"
    
    # تحديد مسار الحفظ المؤقت
    file_path = f"/tmp/{filename}" if filename else f"/tmp/{file_obj.file_unique_id}"
    ...
    try:
        ...
        # 2. تخمين نوع الملف (MIME Type)
        mime_type, _ = mimetypes.guess_type(file_path)
        
        # تخمين احتياطي في حال فشل التخمين التلقائي
        if not mime_type:
            ...
            else: mime_type = 'application/pdf' # افتراض أخير

        logging.info(f"Processing file: {file_path} with type: {mime_type}")

        # 3. رفع الملف إلى Gemini (التصحيح هنا باستخدام config)
        uploaded_file = client.files.upload(
            file=file_path,
            config={'mime_type': mime_type}  # ✅ الحل: وضع النوع داخل config
        )
