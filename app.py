        ...
        try:
            ...
            if "429" in error_msg or "rate_limit_exceeded" in error_msg:
                ...
                if attempt == retries - 1:
                    # تم التعديل هنا لإرجاع رسالة الخطأ الأصلية الخام
                    return error_msg
                continue 
...
async def media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await status.edit_text(f"❌ حدث خطأ أثناء معالجة الطلب: {str(e)}")
    finally:
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # لا نحفظ الرسالة في التاريخ إذا كانت رسالة خطأ
        if "rate limit" not in ai_reply.lower() and "❌" not in ai_reply:
