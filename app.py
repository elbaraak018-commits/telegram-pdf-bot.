...
from pptx import Presentation  # مكتبة للتعامل مع ملفات البوربوينت
...
def extract_text_from_pptx(file_path):
    """استخراج النصوص من ملف بوربوينت"""
    try:
...
نسق الإجابة كالتالي:
1. 📌 عنوان الدرس: (ضع عنواناً واضحاً ومعبراً هنا).
2. 📖 الشرح: (اشرح كل نقطة بالتفصيل الممل وبأسلوب مبسط مع استخدام إيموجيات تعليمية).
3. 💡 ملخص الأفكار: (نقاط مركزة للأهم).
4. ✏️ أمثلة توضيحية: (أمثلة تطبيقية شاملة).
5. 📝 الأسئلة: (أسئلة متنوعة: مقالية، اختيارية، صح وخطأ).
...
- استخدم الإيموجيات بشكل متوازن وبسيط.
- لا تتبع نظام التحليل الدراسي إلا إذا طلب منك ذلك.
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "أرسل لي ملفات PDF أو PowerPoint أو صوراً وسأقوم بتحليلها!\n\n"
...
async def clean_logs_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with engine.connect() as connection:
        ...
        connection.commit()
    await update.message.reply_text("✅ تم تنظيف السجلات.")
...
async def broadcast_execute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ انتهى البث.")
...
# ------------------------------------------------------------------------------
# 7. معالج الوسائط الشامل (تعديل لدعم PPT)
# ------------------------------------------------------------------------------
async def media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if file_size > MAX_FILE_SIZE:
        await msg.reply_text("⚠️ الحد الأقصى 20 ميجابايت.")
        return
    ...
    try:
        if msg.photo:
            ...
            if mime_type == "application/pdf":
                ...
                mode = "pdf"
            elif "presentation" in mime_type or "powerpoint" in mime_type or msg.document.file_name.endswith(('.pptx', '.ppt')):
                file_obj = await msg.document.get_file()
                temp_path = f"temp_{file_obj.file_unique_id}.pptx"
                mode = "pptx"
            else:
                await status.edit_text("❌ نوع الملف غير مدعوم.")
                return
            ...
            await status.edit_text("❌ تنسيق غير معروف.")
            return

        await file_obj.download_to_drive(temp_path)

        if mode == "pptx":
            await status.edit_text("📂 تم استلام ملف بوربوينت...")
            extracted_text = extract_text_from_pptx(temp_path)
            if not extracted_text.strip():
                await status.edit_text("⚠️ الملف لا يحتوي على نص قابل للقراءة.")
                return
            
            context.user_data[f"ppt_text_{msg.from_user.id}"] = extracted_text
            keyboard = [
                [InlineKeyboardButton("تحويل إلى PDF 📄", callback_data="ppt_to_pdf")],
                [InlineKeyboardButton("شرح محتوى الملف 🧠", callback_data="ppt_explain")]
            ]
            await status.delete()
            await msg.reply_text("مرحباً! ماذا تريد أن أفعل بملف البوربوينت؟", reply_markup=InlineKeyboardMarkup(keyboard))

        elif mode == "pdf":
            doc = fitz.open(temp_path)
            extracted_text = "".join([page.get_text() for page in doc])
            doc.close()
            ai_reply = get_ai_response(f"حلل هذا الملف:\n{extracted_text[:60000]}", mode="study_text")
            pdf_file = create_pdf_from_text(ai_reply)
            await status.delete()
            if pdf_file:
                await msg.reply_document(document=open(pdf_file, 'rb'), caption="✅ تم التحليل.")
                os.remove(pdf_file)
            ...
            keyboard = [[InlineKeyboardButton("نص 📝", callback_data="audio_show_text")], [InlineKeyboardButton("شرح 🧠", callback_data="audio_explain_text")]]
            await status.delete()
            await msg.reply_text("تمت معالجة الصوت، اختر:", reply_markup=InlineKeyboardMarkup(keyboard))

    except Exception as e:
        logger.error(f"Error: {e}")
        await status.edit_text("❌ حدث خطأ.")
    finally:
        if temp_path and os.path.exists(temp_path): os.remove(temp_path)

# ------------------------------------------------------------------------------
# 8. معالج أزرار التفاعل (تعديل لدعم PPT)
# ------------------------------------------------------------------------------
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("ppt_"):
        extracted_text = context.user_data.get(f"ppt_text_{user_id}")
        if not extracted_text:
            await query.edit_message_text("⚠️ عذراً، انتهت الجلسة.")
            return

        status_msg = await query.message.reply_text("🚀 جاري التنفيذ...")
        if data == "ppt_to_pdf":
            pdf_file = create_pdf_from_text(extracted_text)
            await status_msg.delete()
            if pdf_file:
                await query.message.reply_document(document=open(pdf_file, 'rb'), caption="✅ تم تحويل ملف البوربوينت إلى PDF.")
                os.remove(pdf_file)
            else: await query.message.reply_text("❌ فشل التحويل.")
        
        elif data == "ppt_explain":
            ai_reply = get_ai_response(f"حلل واشرح هذا الملف الدراسي:\n{extracted_text[:60000]}", mode="study_text")
            pdf_file = create_pdf_from_text(ai_reply)
            await status_msg.delete()
            if pdf_file:
                await query.message.reply_document(document=open(pdf_file, 'rb'), caption="✅ إليك شرح محتوى البوربوينت.")
                os.remove(pdf_file)
            else: await query.message.reply_text(ai_reply)

    # معالجة الصوتيات (القديمة)
        ...
        if data == "audio_show_text":
            ...
            ai_reply = get_ai_response(transcription, mode="study_text")
            pdf_file = create_pdf_from_text(ai_reply)
            if pdf_file:
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    study_keywords = ["اشرح لي", "ما هو شرح", "لخص درس", "حلل النص"]
    is_study = any(k in user_input.lower() for k in study_keywords)
    
    msg_wait = await update.message.reply_text("🤔")
    ai_reply = get_ai_response(user_input, mode="study_text" if is_study else "text")
    await msg_wait.delete()

    if is_study:
        pdf_file = create_pdf_from_text(ai_reply)
        if pdf_file:
            await update.message.reply_document(document=open(pdf_file, 'rb'), caption="✅ إليك الشرح.")
            os.remove(pdf_file)
