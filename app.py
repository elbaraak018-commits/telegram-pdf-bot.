...
def split_text(text, max_len=MAX_TELEGRAM_MESSAGE_LENGTH):
    """
    تقسيم النص الطويل إلى أجزاء أصغر من 4096 حرفًا، 
    مع احترام فواصل الأسطر وتقسيم الأسطر الطويلة نفسها إذا لزم الأمر.
    ...
    # 1. تمرير على النص بالكامل لتقسيم أي سطر أطول من اللازم
    for line in text.splitlines(keepends=True):
        # إذا كان السطر نفسه أطول من الحد الأقصى، قم بتقسيمه إجبارياً
        while len(line) > max_len:
            parts.append(line[:max_len].strip())
            line = line[max_len:]
        
        # إضافة ما تبقى من السطر (أصبح الآن أقل من max_len)
        parts.append(line)
        
    # 2. إعادة دمج الأجزاء الصغيرة الآن إلى رسائل لا تتجاوز الحد الأقصى
    final_parts = []
    current_part = ""
    
    for part in parts:
        # إذا كانت إضافة الجزء الحالي ستتجاوز الحد، قم بحفظ الجزء القديم وابدأ جزءًا جديدًا
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # استخدام الدالة المعدلة لتقسيم النص
    ...
    except error.BadRequest as e: # <--- التقاط خطأ التليجرام (مثل File is too big)
        error_message = str(e)
        ...
        # 🔑 الرسالة القديمة للأخطاء العامة المتبقية
        await status_msg.edit_text(f"❌ حدث خطأ داخلي أثناء المعالجة.")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(q in processed_text for q in ["من انشاك", "من طورك", "من صنعك", "مين سواك"]):
    ...
    try:
        # ✅ تعديل البرومبت ليكون احترافياً، مليئاً بالإيموجي، وخالياً من علامة النجمة (**)
        SIMPLE_TEXT_PROMPT = f"""
      EduVise🌟أنت مساعد ذكي ومحترف واسمك .
        مهمتك هي الرد على المستخدم بأسلوب احترافي جداً وودود.
        
        تعليمات صارمة للتنسيق:
        1. 🎨 استخدم الإيموجي المتنوع والمناسب (مثل: ✅, 💡, 📝, 🚀) لتزيين الرد وجعله حياً، لكن بشكل متناسق.
        2. 🚫 ممنوع منعاً باتاً استخدام علامة النجمة المزدوجة (**) لتغميق النص (Bold). اكتب النصوص بشكل عادي تماماً.
        3. اجعل إجابتك متوازنة (ليست قصيرة جداً ولا طويلة مملة) ومفيدة للمستخدم.

        رسالة المستخدم: {update.message.text}
        """
        response = client.models.generate_content(
            model='gemini-2.5-flash',
        ...
        await msg.edit_text(f"⚠️خطأ: حدث خطأ أثناء معالجة النص.")
