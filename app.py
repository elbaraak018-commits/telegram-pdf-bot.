...
def create_pdf_from_text(content, filename="EduVise_Explanation.pdf"):
    """دالة بسيطة لتحويل النص إلى ملف PDF"""
    c = canvas.Canvas(filename, pagesize=A4)
    width, height = A4
    c.setFont("Helvetica", 12) # ملاحظة: يدعم الإنجليزية بشكل افتراضي، للعربية يفضل استخدام خطوط TTF
    ...
    for line in lines:
        if y < 50:
            ...
            y = height - 50
        # معالجة النصوص الطويلة جداً في السطر الواحد
...
- لا ترسل كلمات أو حروف غريبه فى الرد
-(لا تظهر اسم الصانع والمالك الا بعد سوالك من هو ) منشئ وصانع ومصمم ومطور هذا البوت هو Al-baraa
"""
...
async def media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if mode == "pdf":
            ...
            # تحويل الرد إلى ملف PDF
            pdf_file = create_pdf_from_text(ai_reply, f"EduVise_{msg.from_user.id}.pdf")
            await status.delete()
            ...
            # تحويل الرد إلى ملف PDF
            pdf_file = create_pdf_from_text(ai_reply, f"EduVise_Vision_{msg.from_user.id}.pdf")
            await status.delete()
            await msg.reply_document(document=open(pdf_file, 'rb'), caption="✅ تم تحليل الصورة! إليك الشرح في ملف PDF.")
            os.remove(pdf_file)
        ...
        await status.edit_text(f"❌ حدث خطأ أثناء معالجة الملف.")
    finally:
...
async def audio_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "audio_show_text":
        ...
        pdf_file = create_pdf_from_text(ai_reply, f"EduVise_Audio_{user_id}.pdf")
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # إذا كان الطلب دراسياً، أرسل PDF. إذا كانت دردشة عادية، أرسل نصاً.
        if is_study_request:
            pdf_file = create_pdf_from_text(ai_reply, f"EduVise_Study_{user_id}.pdf")
