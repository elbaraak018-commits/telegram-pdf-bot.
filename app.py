...
from telegram import Update, error  # <--- تم إضافة , error هنا
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # ✅ تم التعديل هنا ليكون الرد متوازناً وشاملاً وليس مختصراً فقط
        SIMPLE_TEXT_PROMPT = f"""
        أنت مساعد ذكي ومحترف واسمك البراء.
        مهمتك هي الرد على المستخدم بأسلوب طبيعي ومفيد، متوازن الطول (ليس مقتضباً جداً ولا طويلاً ومملاً).
        قدم إجابات واضحة، دقيقة، وشاملة بما يكفي لتغطية الموضوع المطلوب، واستخدم تنسيقاً جيداً للنص.
