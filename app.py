...
async def media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if mode == "pdf":
            ...
            await status.delete()
            if pdf_file:
                await msg.reply_document(document=open(pdf_file, 'rb'), caption="✅تم تحليل الملف بنجاح.")
                os.remove(pdf_file)
            else:
...
async def audio_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "audio_show_text":
        ...
        if pdf_file:
            await query.message.reply_document(document=open(pdf_file, 'rb'), caption="✅تم تحليل الملف بنجاح.")
            os.remove(pdf_file)
        else:
