...
# ----------------------------------------
# 9. الدمج والتشغيل (تم تعديل هذا الجزء للعمل مع Mini App)
# ----------------------------------------

# 🛠️ تهيئة Flask
flask_app = Flask(__name__, static_folder='webapp')
app: Application = None # تعريف عالمي لتطبيق البوت

# 1. مسار لخدمة التطبيق المصغر (Mini App)
@flask_app.route('/')
def serve_webapp():
    """خدمة ملف index.html عندما يفتح المستخدم التطبيق المصغر."""
    # يجب أن يكون ملف index.html داخل مجلد webapp
...
def main():
    ...
    # في بيئات مثل Render، سيتم تشغيل هذا الخادم بواسطة Gunicorn (يجب التأكد من ملف requirements.txt)
