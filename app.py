...
# ----------------------------------------
# 9. الدمج والتشغيل (تم تعديل هذا الجزء لحل خطأ 500)
# ----------------------------------------
...
def telegram_webhook(): # ⚠️ تم حذف async لتجنب مشكلة التزامن مع Gunicorn/Flask
    ...
    try:
        # تحويل JSON Request إلى كائن Update
        update = Update.de_json(data=request.get_json(force=True), bot=app.bot)
        
        # 💡 الحل: تشغيل دالة app.process_update غير المتزامنة في Thread Pool 
        # لعدم حظر خادم Flask/Gunicorn الرئيسي
        def process_update_sync():
            # استخدام asyncio.run لتشغيل الدالة غير المتزامنة ضمن الثريد الجديد
            asyncio.run(app.process_update(update))

        thread = threading.Thread(target=process_update_sync)
        thread.start()
        
        # يتم الرد فوراً بـ OK لتجنب انتهاء مهلة تيليجرام، بينما يتم العمل في الخلفية
        return "OK"
...
def main():
    ...
    # 1. إعداد تطبيق تيليجرام (Handlers)
    ...
    app.add_handler(broadcast_handler) # تم تعريفها مسبقاً
    ...
    # 2. إعداد Webhook عند بدء التشغيل
    ...
    # 3. تشغيل خادم Flask لخدمة الـ Mini App واستقبال Webhook
