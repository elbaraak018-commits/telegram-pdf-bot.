...
def telegram_webhook():
    ...
    if not app:
        # إذا لم يتم تهيئة تطبيق البوت، يتم الرد بخطأ الخادم
        return "Bot Application not initialized", 500
    
    try:
        # 1. تحويل JSON إلى Update
        update = Update.de_json(data=request.get_json(force=True), bot=app.bot)

        # 2. تعريف الدالة التي ستعمل في ثريد الخلفية
        def run_async_process(update_obj):
            """تشغيل عملية معالجة التحديث في سياق غير متزامن منفصل."""
            try:
                # استخدام asyncio.run لتشغيل الدالة غير المتزامنة ضمن الثريد الجديد
                asyncio.run(app.process_update(update_obj))
            except Exception as e:
                # يجب تسجيل الخطأ هنا، وليس في الخيط الرئيسي
                logging.error(f"Error in async thread processing update: {e}")

        # 3. تشغيل الدالة في ثريد منفصل
        # **هذا يضمن أن يرد الخادم فوراً على تيليجرام قبل بدء المعالجة الطويلة**
        thread = threading.Thread(target=run_async_process, args=(update,))
        thread.start()
        
        # 4. الرد فوراً على تيليجرام
        return "OK"
