...
# 🔑 الدالة الجديدة لعرض سجل الرسائل
async def get_message_logs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """يعرض آخر 10 رسائل مستلمة من قاعدة البيانات والعدد الإجمالي (للمدير فقط)."""
    if update.effective_user.id != ADMIN_ID:
        return
    if not engine:
    ...
    response += "📜 **آخر 10 رسائل مستلمة (من الأحدث للأقدم):**\n"
    response += "-----------------------\n"
