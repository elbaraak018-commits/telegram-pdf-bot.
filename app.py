...
async def media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if msg.photo:
            ...
            # التحقق من نوع المستند (ندعم PDF فقط حالياً)
        ...
        if mode == "pdf":
            ...
            if not extracted_text.strip():
                await status.edit_text("⚠️ يبدو أن ملف PDF فارغ أو يحتوي على صور فقط لا يمكنني قراءتها كنص حالياً.")
                return
...
def main():
    ...
    # تم تعديل الفلتر ليشمل جميع المستندات للتحقق من النوع داخل الميدياً هاندلر
