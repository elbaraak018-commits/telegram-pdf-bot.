...
# ✅ تم تعديل هذه الدالة لاستخدام الاسم الدقيق للنسخة المستقرة لتجنب خطأ 404
def get_or_create_chat(user_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_key not in context.user_data:
        ...
        context.user_data[chat_key] = client.chats.create(
            model='gemini-1.5-flash-002', # <--- تم التعديل إلى النسخة المستقرة المحددة
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ✅ تم التعديل لاستخدام النسخة المستقرة المحددة (1.5 Flash 002)
        response = client.models.generate_content(
            model='gemini-1.5-flash-002', # <--- تم التعديل هنا
