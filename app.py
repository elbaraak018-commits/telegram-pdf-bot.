...
from telegram import Update, error, WebAppInfo, ReplyKeyboardMarkup, KeyboardButton 
...
# 🔥 الرابط الجديد لـ Netlify
WEB_APP_URL = "https://regal-palmier-acb667.netlify.app/" 
...
# 🔑 الدالة لتسجيل محتوى الرسالة في جدول messages
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إنشاء زر لوحة المفاتيح
    keyboard = [
        [KeyboardButton("💬 فتح المحادثة الذكية", web_app=web_app_info)]
    ]
    
    # تغليف الزر في لوحة مفاتيح الردود
...
اضغط على الزر **"💬 فتح المحادثة الذكية"** (أسفل الشاشة) للبدء في الدردشة الذكية بواجهة مخصصة وسهلة الاستخدام! 🚀

أو يمكنك إرسال الملفات هنا مباشرة للتحليل.
"""
...
# ----------------------------------------
# 8. معالج بيانات Web App
# ----------------------------------------
async def handle_web_app_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """يستقبل البيانات المرسلة من واجهة الويب ويغذيها لـ Gemini."""
    if not update.message.web_app_data:
        return

    user_id = update.effective_user.id
    data = update.message.web_app_data.data
    log_message(user_id, f"WebApp: {data}", 'web_app_chat')

    if data.startswith("TEXT_CHAT:"):
        user_text = data.split("TEXT_CHAT:", 1)[1]
        
        chat = get_or_create_chat(user_id, context)
        if not chat: 
            await update.message.reply_text("البوت غير مفعل.")
            return
        ...
        try:
            ...
            try:
                await msg.edit_text("⚠️ حدث خطأ أثناء معالجة رسالة Web App.")
            except:
                await update.message.reply_text("⚠️ حدث خطأ أثناء معالجة رسالة Web App.")

# ----------------------------------------
# 9. معالج النصوص (المعدل والمحسن)
# ----------------------------------------
        ...
        for i, part in enumerate(response_parts):
            ...
            if i == 0:
                try:
                    ...
                    logging.error(f"Failed to edit first part in handle_text.")
                    await update.message.reply_text(final_content)
            else:
                await update.message.reply_text(final_content)
                time.sleep(0.3) 
