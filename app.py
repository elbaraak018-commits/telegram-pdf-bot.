...
def log_message(user_id, content, msg_type):
    ...
    try:
        with engine.connect() as connection:
            connection.execute(query, {"user_id": user.id, "content": content_to_log, "msg_type": msg_type})
            connection.commit()
    except SQLAlchemyError as e:
    ...
    try:
        with engine.connect() as connection:
            ...
            await msg.edit_text(
                ...
                f"لم يتم المساس بجدول المستخدمين (users)."
            )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔑 التعديل الجديد: الرد "البراء 🙂" عند السؤال عن الاسم أو المنشئ
    if ("ما اسمك" in processed_text or "من انت" in processed_text or 
        "من انشأك" in processed_text or "من طورك" in processed_text or "من صممك" in processed_text):
